...
class _AdminDashboardScreenState extends ConsumerState<AdminDashboardScreen> {
  ...
  String _title(dynamic item) {
    ...
    if (section == 'reports') return _reportTargetLabel(item['targetType'] as String?);
  ...
  String _subtitle(dynamic item) {
    if (section == 'reports') return '${item['reasonCode'] ?? ''} — ${item['reason'] ?? ''}';
    if (section == 'banned-words') return 'Filtré automatiquement de toute publication ou commentaire';
      ...
      case 'reports':
        ...
        return <PopupMenuEntry<String>>[
          PopupMenuItem<String>(
            ...
            onTap: () => _act(
              () => api.patch<void>('/moderation/reports/${item['id']}', data: <String, String>{'status': 'dismissed'}),
          ...
          PopupMenuItem<String>(
            ...
            onTap: () => _act(
              () => api.patch<void>(
                ...
                data: <String, String>{'status': 'actioned', 'action': 'content_removed'},
              ),
              'Contenu traité.',
            ),
          ),
          if (targetType == 'user') ...<PopupMenuEntry<String>>[
            ...
            PopupMenuItem<String>(
              ...
              onTap: () => _resolveWithSanction(item, 'suspension', suspensionDays: 7),
            ),
            PopupMenuItem<String>(
              value: 'ban',
  ...
  Future<void> _resolveWithSanction(dynamic report, String action, {int? suspensionDays}) => _act(
  ...
  Future<void> _addBannedWord() async {
    ...
    await _act(
      () => ref.read(apiClientProvider).post<void>('/moderation/banned-words', data: <String, String>{'word': word}),
