...
/// qui n'existent pas.
const _summarySections = <String>{'commissions', 'revenue', 'delivery-fees', 'reports'};
...
class _FinanceAdminScreenState extends ConsumerState<FinanceAdminScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      if (_summarySections.contains(section)) {
        summary = (data is Map && data['data'] is Map) ? Map<String, dynamic>.from(data['data'] as Map) : const {};
        rows = const [];
      } else {
        rows = data is Map && data['data'] is List ? data['data'] as List : const [];
        summary = const {};
      }
      loading = false;
    });
  ...
  Future<void> _act(Future<void> Function() action, String successMessage) async {
    ...
    try {
      ...
      final message = body is Map<String, dynamic> ? ((body['error'] as Map<String, dynamic>?)?['message'] as String?) : null;
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        floatingActionButton: section == 'refunds'
            ? FloatingActionButton(onPressed: _createRefund, child: const Icon(Icons.add))
            : null,
        body: Column(
          children: <Widget>[
            ...
            Expanded(
              child: loading
                  ...
                  : _summarySections.contains(section)
                      ? _SummaryView(section: section, summary: summary, onRefresh: _load)
                      : rows.isEmpty
                          ? const Center(child: Text('Rien à afficher dans cette section.'))
                          : RefreshIndicator(
                              onRefresh: _load,
                              child: ListView.builder(
                                itemCount: rows.length,
                                itemBuilder: (_, index) => _rowTile(rows[index] as Map<String, dynamic>),
                              ),
                            ),
            ),
          ],
    ...
    switch (section) {
      case 'transactions':
        return ListTile(
          ...
          title: Text('${item['amount']} Ar — ${_transactionLabel(item['type'] as String?)}'),
          ...
          trailing: status == 'pending'
              ? PopupMenuButton<String>(
                  itemBuilder: (_) => const <PopupMenuEntry<String>>[
                    PopupMenuItem<String>(value: 'paid', child: Text('Marquer payé')),
                    PopupMenuItem<String>(value: 'rejected', child: Text('Rejeter')),
                  ],
                  onSelected: (value) {
                    final kind = section == 'merchant-withdrawals' ? 'merchants' : 'couriers';
                    final id = (item['id'] ?? item['_id']).toString();
                    _act(
                      () => ref.read(apiClientProvider).patch<void>('/finance/withdrawals/$kind/$id', data: <String, String>{'status': value}),
                      value == 'paid' ? 'Retrait marqué payé.' : 'Retrait rejeté.',
                    );
                  },
                )
              : Text(status ?? ''),
  ...
  Future<void> _createRefund() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: <Widget>[
            TextField(controller: orderId, decoration: const InputDecoration(labelText: 'Identifiant de la commande')),
            TextField(
              controller: amount,
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Montant (Ar) — vide = total de la commande'),
            ),
        ...
        actions: <Widget>[
          ...
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Rembourser')),
        ],
      ),
    );
    ...
    await _act(
      () => ref.read(apiClientProvider).post<void>('/finance/refunds', data: <String, dynamic>{
        'orderId': orderId.text.trim(),
        if (amount.text.trim().isNotEmpty) 'amount': double.tryParse(amount.text.trim()),
        'reason': reason.text.trim().isEmpty ? 'Remboursement administratif' : reason.text.trim(),
      }),
      'Remboursement émis.',
    );
  }
...
class _SummaryView extends StatelessWidget {
  const _SummaryView({required this.section, required this.summary, required this.onRefresh});
        ...
        children: <Widget>[
          for (final tile in tiles)
            Card(
              child: ListTile(
                title: Text(tile.$1),
                trailing: Text(tile.$2, style: Theme.of(context).textTheme.titleMedium),
              ),
            ),
        ],
      ),
  ...
  List<(String, String)> _revenueTiles() => <(String, String)>[
        ...
        ('Revenu frais de livraison', '${summary['deliveryFeeRevenue'] ?? 0} Ar'),
        ('Remboursé', '${summary['refunded'] ?? 0} Ar'),
        ('Revenu net de plateforme', '${summary['netPlatformRevenue'] ?? 0} Ar'),
      ];

  List<(String, String)> _reportTiles() {
    final byType = summary['byType'] as Map<String, dynamic>? ?? const {};
    final merchantWithdrawals = summary['merchantWithdrawalsPaid'] as Map<String, dynamic>? ?? const {};
    final courierWithdrawals = summary['courierWithdrawalsPaid'] as Map<String, dynamic>? ?? const {};
    final tiles = <(String, String)>[
      ('Période', '${summary['from'] ?? ''} → ${summary['to'] ?? ''}'),
      ('Remboursements émis', '${summary['refundsIssued'] ?? 0}'),
      ('Retraits commerçants payés', '${merchantWithdrawals['total'] ?? 0} Ar (${merchantWithdrawals['count'] ?? 0})'),
      ('Retraits livreurs payés', '${courierWithdrawals['total'] ?? 0} Ar (${courierWithdrawals['count'] ?? 0})'),
    ];
    for (final entry in byType.entries) {
      final value = entry.value as Map<String, dynamic>;
      tiles.add(('Mouvements « ${entry.key} »', '${value['total']} Ar (${value['count']})'));
    }
    return tiles;
  }
}
