...
class MerchantStockScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MerchantStockScreen> createState() => _MerchantStockScreenState();
}

class _MerchantStockScreenState extends ConsumerState<MerchantStockScreen>
    with SingleTickerProviderStateMixin {
  late final TabController _tab = TabController(length: 2, vsync: this);
  late Future<List<Map<String, dynamic>>> _alerts = _loadAlerts();
  late Future<List<Map<String, dynamic>>> _history = _loadHistory();

  Future<List<Map<String, dynamic>>> _loadAlerts() async {
    final response = await ref
        .read(apiClientProvider)
        .get<Map<String, dynamic>>('/shop/${widget.shopId}/stock/alerts');
  ...
  Future<List<Map<String, dynamic>>> _loadHistory() async {
    final response = await ref
        .read(apiClientProvider)
        .get<Map<String, dynamic>>('/shop/${widget.shopId}/stock/movements');
    final data = response.data?['data'];
    return data is List ? data.whereType<Map<String, dynamic>>().toList() : const [];
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: TabBarView(
          controller: _tab,
          children: <Widget>[
            _AlertsList(future: _alerts, onRefresh: () async => _refresh()),
            _HistoryList(future: _history, onRefresh: () async => _refresh()),
          ],
        ),
      );
  ...
  Future<void> _recordMovement(BuildContext context) async {
    ...
    final products = ((productsResponse.data?['data'] as List<dynamic>?) ?? const <dynamic>[])
        .whereType<Map<String, dynamic>>()
        .toList();
    if (!context.mounted) return;
    if (products.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Ajoutez d’abord un produit à votre catalogue.')),
      );
      return;
    }

    String? productId = (products.first['id'] ?? products.first['_id']).toString();
    String type = 'in';
              ...
              children: <Widget>[
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const <DropdownMenuItem<String>>[
                    ...
                    DropdownMenuItem(value: 'correction', child: Text('Correction (stock réel)')),
                  ],
    ...
    if (productId == null || parsedQuantity == null || reasonText.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Quantité et motif sont requis.')),
      );
      return;
    }
    ...
    try {
      ...
      _refresh();
      messenger.showSnackBar(const SnackBar(content: Text('Mouvement enregistré.')));
      ...
      messenger.showSnackBar(
        SnackBar(content: Text(message ?? 'Impossible d’enregistrer ce mouvement.')),
      );
    }
  }
}
...
class _AlertsList extends StatelessWidget {
  const _AlertsList({required this.future, required this.onRefresh});
              ...
              child: ListView(
                children: const <Widget>[
                  Padding(
                    ...
                    child: Center(child: Text('Aucune alerte de stock.')),
                  ),
                ],
              ),
            );
...
class _HistoryList extends StatelessWidget {
  const _HistoryList({required this.future, required this.onRefresh});
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Text('${movement['stockBefore'] ?? '?'} → ${movement['stockAfter'] ?? '?'}'),
                );
              },
            ),
          );
